...
# 🚀 FileShare v1.0.1 更新日志

**轻量级文件分享系统** — 支持目录分享、提取码保护、在线预览

> 本次更新重点简化了安装流程，并新增了多项系统设置与安全功能。

## 🆕 新特性

### 首次访问初始化页面
- 安装后，首次访问后台 (`<IP>:6606/admin`) 将自动弹出 **初始化管理员页面**。
- 用户可直接在此页面设置管理员用户名和密码，**无需记忆或查找默认账号**。

### 安装流程简化
- 移除了安装向导中需要手动输入端口和管理员密码的步骤。
- 运行端口默认为 **6606**。
- 管理员账号在首次 Web 访问时设置，安装体验更加顺畅。
...
- ✅ **新增** 系统安全设置，包括：
    ...
    - 单次下载数量限制
    - 提取码长度与字符类型限制
- ✅ **新增** 模板设置，支持自定义分享页和404页面。
- ✅ **新增** 通知设置，可在用户进入分享页时弹出网页通知。
...
| 文件 | 适用平台 | 说明 |
| :--- | :--- | :--- |
| `FileShare_1.0.1.zip` | Linux | 解压后，执行 `bash run.sh start` 即可运行。 |
| `fileshare_1.0.1_x86.fpk` | fnOS | 飞牛 NAS 应用安装包。 |

> **注意**：如需保留旧版本数据，请在升级前备份 `data/fileshare.db` 文件。

---
*发布日期：2026年5月19日*  
**完整更新日志**: https://github.com/cjm2004/FileShare[/compare/v1.0.0...v1.0.1]
