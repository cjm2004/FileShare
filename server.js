...
const app = express();
const PORT = process.env.PORT || process.env.TRIM_SERVICE_PORT || 6606;
...
const logger = { info: (m,d) => log('INFO',m,d), warn: (m,d) => log('WARN',m,d), error: (m,d) => log('ERROR',m,d), debug: (m,d) => log('DEBUG',m,d) };

// ── 🔒 Dynamic regex builder for share/extract codes ──
function buildCodeRegex(charsSetting) {
  const parts = (charsSetting || 'en,num').split(',');
  let cls = '';
  if (parts.includes('en')) cls += 'a-zA-Z';
  if (parts.includes('num')) cls += '0-9';
  if (parts.includes('zh')) cls += '\\u4e00-\\u9fa5';
  if (!cls) cls = 'a-zA-Z0-9';
  return new RegExp('^[' + cls + '_-]+$');
}
...
// ── 🔒 Constant-time string comparison (防时序攻击) ──
...
function getMaxLoginAttempts() {
  return Math.max(1, parseInt(getSetting('rate_limit_login_max', '5')));
}
function checkLoginLockout(ip, username) {
  const key = `${ip}:${username}`;
  const record = loginAttempts.get(key);
  if (record && record.count >= getMaxLoginAttempts()) {
    const elapsed = (Date.now() - record.since) / 1000 / 60;
    if (elapsed < LOGIN_LOCKOUT_MINUTES) {
      return { locked: true, remainingMinutes: Math.ceil(LOGIN_LOCKOUT_MINUTES - elapsed) };
    }
...
function recordLoginAttempt(ip, username) {
  const key = `${ip}:${username}`;
  const record = loginAttempts.get(key) || { count: 0, since: Date.now() };
  record.count++;
  if (record.count === 1) record.since = Date.now();
  loginAttempts.set(key, record);
}
function resetLoginAttempts(ip, username) {
...
// ── SQLite (sql.js — pure JS, no native compilation needed) ──
const initSqlJs = require('sql.js/dist/sql-asm.js');

let sqldb = null;

// ── sql.js helpers (synchronous wrappers) ──
...
function dbRunNoSave(sql, params = []) {
  sqldb.run(sql, params);
}

// ── Persistence: save in-memory DB to disk ──
function saveDb() {
  const data = sqldb.export();
  fs.writeFileSync(DB_PATH, Buffer.from(data));
}

// ── Init tables (executed inside initDatabase) ──
const SCHEMA_SQL = `
...
async function initDatabase() {
  ...
  // Migration: add showNotification column if missing
  try { sqldb.run('ALTER TABLE shares ADD COLUMN showNotification TEXT DEFAULT "true"'); } catch(e) {}
  saveDb();
  logger.info('SQLite database ready', { size: fs.existsSync(DB_PATH) ? fs.statSync(DB_PATH).size : 0 });
}

// ── DB Helper (synchronous, dbGet/dbAll/dbRun wrappers) ──
...
function getShares() {
  const rows = dbAll('SELECT * FROM shares ORDER BY rowid DESC');
  return rows.map(r => ({ ...r, files: JSON.parse(r.files || '[]') }));
}
function getShare(code) {
  const r = dbGet('SELECT * FROM shares WHERE code=?', [code]);
  if (!r) return null;
  return { ...r, files: JSON.parse(r.files || '[]') };
}
function addShare(data) {
  dbRun(`INSERT INTO shares (id,code,files,extractCode,expiryDate,maxVisits,visitCount,topHint,bottomHint,createdAt,createdBy,showNotification) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`, [
    ...
    data.showNotification !== 'false' ? 'true' : 'false'
  ]);
}
function deleteShare(id) {
  dbRun('DELETE FROM shares WHERE id=?', [id]);
}
function updateShare(id, fields) {
  const allowed = ['extractCode', 'expiryDate', 'maxVisits', 'topHint', 'bottomHint', 'showNotification'];
  const keys = Object.keys(fields).filter(k => allowed.includes(k));
  if (keys.length === 0) return;
  const sets = keys.map(k => `${k}=?`).join(',');
  const vals = keys.map(k => fields[k]);
  dbRun(`UPDATE shares SET ${sets} WHERE id=?`, [...vals, id]);
}
function incVisit(code, req) {
  dbRun('UPDATE shares SET visitCount=visitCount+1 WHERE code=?', [code]);
  // Record hourly visit log
  var now = new Date();
  var dateStr = now.getFullYear() + '-' +
    String(now.getMonth() + 1).padStart(2, '0') + '-' +
    String(now.getDate()).padStart(2, '0');
  var hour = now.getHours();
  dbRun('INSERT INTO visit_logs (date, hour, share_code, created_at) VALUES (?,?,?,?)',
    [dateStr, hour, code, now.toISOString()]);
}
...
function addUser(username, hash) {
  dbRun('INSERT OR REPLACE INTO users (username,passwordHash) VALUES (?,?)', [username, hash]);
}
function updateUser(oldName, newName, hash) {
  if (hash) dbRun('UPDATE users SET username=?, passwordHash=? WHERE username=?', [newName, hash, oldName]);
  else dbRun('UPDATE users SET username=? WHERE username=?', [newName, oldName]);
}
...
app.use((req, res, next) => {
  ...
  // CSP: allow inline scripts (needed for share.html templates) but block external
  res.setHeader('Content-Security-Policy', "default-src 'self'; img-src 'self' data: https: http:; style-src 'self' 'unsafe-inline' https:; script-src 'self' 'unsafe-inline'; connect-src 'self' https: http:; font-src 'self'; media-src 'self' https: http: blob:; frame-src 'self' https: http:; object-src 'none'");
  next();
});
...
function getTemplateContent(name) {
  const override = path.join(TEMPLATES_DIR, name);
...
// ── List customizable template files ──
function listTemplateFiles() {
  const files = ['share.html', '404.html'];
  const result = [];
  for (const f of files) {
    const overridden = fs.existsSync(path.join(TEMPLATES_DIR, f));
    result.push({ name: f, overridden });
  }
  return result;
}

// ── Get template display name (Chinese) ──
...
function getBaseUrl(req) {
  var d = getSetting('domain', '');
  if (d) {
    // Manual domain configured; determine protocol heuristically
    // If it looks like a bare domain (no port), assume HTTPS; otherwise HTTP
    var proto = d.includes(':') ? 'http://' : 'https://';
    return proto + d;
  }
  // Auto-detect from request when domain setting is empty
  if (req) {
...
// ── Init (called after DB is ready, inside START block) ──
async function appInit() {
  const users = getUsers();
  if (users.length === 0) {
    addUser('admin', await bcrypt.hash('admin123', 10));
    console.log('[Init] 管理员已创建（请立即修改密码）');
  }
  if (!fs.existsSync(SHARED_DIR)) fs.mkdirSync(SHARED_DIR, { recursive: true });
  if (!fs.existsSync(UPLOADS_DIR)) fs.mkdirSync(UPLOADS_DIR, { recursive: true });
  // 🔒 Restrict DB file permissions (owner only)
  try { fs.chmodSync(DB_PATH, 0o600); } catch(e) {}
  try { fs.chmodSync(JWT_SECRET_FILE, 0o600); } catch(e) {}
  console.log('[Init] FileShare 初始化完成');
}
...
app.get('/s/:shareCode', (req, res) => {
  ...
  if (!share) {
    const html = getTemplateContent('share.html')
  ...
  const s = { siteName: getSetting('siteName','文件分享'), backgroundImage: getSetting('backgroundImage',''), announcement: getSetting('announcement',''), footer: getSetting('footer',''), domain: getSetting('domain',''), previewExtensions: getSetting('preview_extensions',''), kkfileviewUrl: getSetting('kkfileview_url',''), previewMode: getSetting('preview_mode','system'), captchaEnabled: getSetting('captcha_enabled','true'), captchaOnExtract: getSetting('captcha_on_extract','true'), captchaOnDownload: getSetting('captcha_on_download','true'), notificationTitle: getSetting('notification_title','') || '欢迎访问 ' + (getSetting('siteName','文件分享')), notificationContent: getSetting('notification_content','') || '感谢您访问本分享，请按需下载文件。', notificationWaitTime: parseInt(getSetting('notification_wait_time','0')) || 0 };
  const html = getTemplateContent('share.html')
    .replace('<!--SHARE_DATA-->', JSON.stringify({ code: share.code, files: share.files, hasExtractCode: !!share.extractCode, topHint: share.topHint || '', bottomHint: share.bottomHint || '', created: share.createdAt, showNotification: share.showNotification !== 'false' }))
...
app.post('/api/share/:shareCode/verify', (req, res) => {
  ...
  // 🔒 Rate limit: configurable verify attempts/minute/IP
  var vLimit = parseInt(getSetting('rate_limit_verify_max', '40'));
  var vEnabled = getSetting('rate_limit_verify_enabled', 'true') === 'true';
  if (vEnabled) {
    const rl = rateLimit('verify:' + ip, vLimit, 60000);
    if (!rl.allowed) {
  ...
  if (share.extractCode) {
    ...
    if (!c || !safeCompare(c, share.extractCode)) return res.status(403).json({ error: '提取码错误' });
  }
  incVisit(code, req);
...
function getShareFile(shareCode, fileName) {
  const share = getShare(shareCode);
  if (!share) return null;
  // Some HTTP clients (KKFileView) double-encode URLs; try both single and double decode
  var tries = [fileName];
  try {
    var decodedOnce = decodeURIComponent(fileName);
    if (decodedOnce !== fileName) tries.push(decodedOnce);
    var decodedTwice = decodeURIComponent(decodedOnce);
    if (decodedTwice !== decodedOnce) tries.push(decodedTwice);
  } catch(e) { /* ignore decode errors */ }
  var f = null;
  for (var i = 0; i < tries.length; i++) {
    f = share.files.find(x => x.name === tries[i]);
    if (f) break;
  }
  if (!f || !f.path || typeof f.path !== 'string') return null;
  if (!fs.existsSync(f.path)) return null;
  // 🔒 Ensure the resolved path doesn't escape via symlinks
  try {
    const real = fs.realpathSync(f.path);
    if (!f.path.startsWith('/')) return null; // must be absolute
    return f;
  } catch {
...
app.get('/api/share/:shareCode/download/*', (req, res) => {
  ...
  const f = getShareFile(req.params.shareCode, fileName);
...
app.post('/api/share/:shareCode/download-batch', (req, res) => {
  ...
  var batchLimit = parseInt(getSetting('batch_download_limit', '100'));
  if (files.length > batchLimit) return res.status(400).json({ error: '超出最大下载文件数：' + batchLimit });
...
app.get('/404.html', (req, res) => {
  const html = getTemplateContent('404.html');
...
// ── 🔒 Security settings ──
app.get('/api/admin/security-settings', auth, (req, res) => {
  const s = {
    rateLimitShareEnabled: getSetting('rate_limit_share_enabled', 'true'),
    rateLimitShareMax: getSetting('rate_limit_share_max', '20'),
    rateLimitVerifyEnabled: getSetting('rate_limit_verify_enabled', 'true'),
    rateLimitVerifyMax: getSetting('rate_limit_verify_max', '40'),
    rateLimitLoginEnabled: getSetting('rate_limit_login_enabled', 'true'),
    rateLimitLoginMax: getSetting('rate_limit_login_max', '5'),
    batchDownloadLimit: getSetting('batch_download_limit', '100'),
    extractMaxLength: getSetting('extract_max_length', '64'),
    shareCodeMaxLength: getSetting('share_code_max_length', '32'),
    extractChars: getSetting('extract_chars', 'en,num'),
    shareChars: getSetting('share_chars', 'en,num')
  };
  res.json(s);
});

app.put('/api/admin/security-settings', auth, (req, res) => {
  const f = req.body;
  const m = {
    rate_limit_share_enabled: f.rateLimitShareEnabled !== undefined ? (f.rateLimitShareEnabled ? 'true' : 'false') : undefined,
    rate_limit_share_max: f.rateLimitShareMax !== undefined ? String(f.rateLimitShareMax) : undefined,
    rate_limit_verify_enabled: f.rateLimitVerifyEnabled !== undefined ? (f.rateLimitVerifyEnabled ? 'true' : 'false') : undefined,
    rate_limit_verify_max: f.rateLimitVerifyMax !== undefined ? String(f.rateLimitVerifyMax) : undefined,
    rate_limit_login_enabled: f.rateLimitLoginEnabled !== undefined ? (f.rateLimitLoginEnabled ? 'true' : 'false') : undefined,
    rate_limit_login_max: f.rateLimitLoginMax !== undefined ? String(f.rateLimitLoginMax) : undefined,
    batch_download_limit: f.batchDownloadLimit !== undefined ? String(f.batchDownloadLimit) : undefined,
    extract_max_length: f.extractMaxLength !== undefined ? String(f.extractMaxLength) : undefined,
    share_code_max_length: f.shareCodeMaxLength !== undefined ? String(f.shareCodeMaxLength) : undefined,
    extract_chars: f.extractChars !== undefined ? f.extractChars : undefined,
    share_chars: f.shareChars !== undefined ? f.shareChars : undefined
  };
  try {
    ...
    logger.error('Save security settings failed', { error: e.message, stack: (e.stack||'').split('\n').slice(0,3).join('|') });
...
app.post('/api/admin/login', async (req, res) => {
  ...
  // 🔒 Rate limit: configurable requests/minute/IP
  var lLimit = parseInt(getSetting('rate_limit_login_max', '5'));
  var lEnabled = getSetting('rate_limit_login_enabled', 'true') === 'true';
  if (lEnabled) {
    const rl = rateLimit('login:' + ip, Math.max(lLimit, 3), 60000);
    if (!rl.allowed) {
      return res.status(429).json({ error: '请求过于频繁，请稍后再试', retryAfter: rl.retryAfter });
    }
  }

  // 🔒 Lockout check: configurable failed attempts = lock for 15 min
  const lock = checkLoginLockout(ip, username);
  if (lock.locked) {
    return res.status(429).json({ error: `登录已锁定，${lock.remainingMinutes}分钟后重试` });
  }
  ...
  const token = jwt.sign({ username, ip: ip.slice(0,40) }, JWT_SECRET, { expiresIn: '4h' });
...
app.get('/api/admin/settings', auth, (req, res) => {
  ...
  s.notification_title = s.notification_title || ('欢迎访问 ' + (s.siteName || '文件分享'));
  s.notification_content = s.notification_content || '感谢您访问本分享，请按需下载文件。';
  s.notification_wait_time = s.notification_wait_time || '0';
  res.json(s);
});

app.put('/api/admin/settings', auth, (req, res) => {
  const { siteName, announcement, footer, scanRootPath, backgroundImage, domain, preview_extensions, kkfileview_url, preview_mode, notification_title, notification_content, notification_wait_time } = req.body;
  const pairs = { siteName, announcement, footer, scanRootPath, backgroundImage, domain, preview_extensions, kkfileview_url, preview_mode, notification_title, notification_content, notification_wait_time };
...
app.put('/api/admin/password', auth, async (req, res) => {
  ...
  if (!oldPassword || !newPassword || newPassword.length < 8) return res.status(400).json({ error: '密码至少8位' });
  if (!/[A-Z]/.test(newPassword) && !/[a-z]/.test(newPassword)) return res.status(400).json({ error: '密码需包含字母' });
  if (!/[0-9]/.test(newPassword)) return res.status(400).json({ error: '密码需包含数字' });
  const user = getUser(req.user.username);
...
const ALLOWED_UPLOAD_EXTS = ['.jpg','.jpeg','.png','.gif','.webp'];
...
app.post('/api/admin/share/create', auth, (req, res) => {
  try {
    const ip = req.ip || req.connection.remoteAddress;
    // 🔒 Rate limit: configurable shares/minute/IP
    var sLimit = parseInt(getSetting('rate_limit_share_max', '20'));
    var sEnabled = getSetting('rate_limit_share_enabled', 'true') === 'true';
    if (sEnabled) {
      const rl = rateLimit('createShare:' + ip, sLimit, 60000);
      if (!rl.allowed) {
        return res.status(429).json({ error: '创建过于频繁，请稍后再试' });
      }
    }

    const { filePaths, extractCode, expiryDate, maxVisits, topHint, bottomHint, appendCode, customCode, showNotification } = req.body;
    ...
    if (code) {
      // Validate: dynamic char sets and max length from settings
      var scMaxLen = parseInt(getSetting('share_code_max_length', '32'));
      var scChars = getSetting('share_chars', 'en,num');
      var scRegex = buildCodeRegex(scChars);
      if (code.length < 2 || code.length > scMaxLen || !scRegex.test(code)) {
        return res.status(400).json({ error: '自定义分享码格式无效（长度2-' + scMaxLen + '位，字符范围不符合设置）' });
      }
      if (getShare(code)) {
        return res.status(400).json({ error: '该分享码已被使用，请换一个' });
      }
    } else {
      do { code = genCode(); } while (getShare(code));
    }
    if (extractCode) {
      var ecMaxLen = parseInt(getSetting('extract_max_length', '64'));
      if (extractCode.length > ecMaxLen) return res.status(400).json({ error: '提取码过长（最多' + ecMaxLen + '位）' });
      var ecChars = getSetting('extract_chars', 'en,num');
      var ecRegex = buildCodeRegex(ecChars);
      if (!ecRegex.test(extractCode)) return res.status(400).json({ error: '提取码包含不允许的字符' });
    }
    const share = { id: Date.now().toString(36)+Math.random().toString(36).slice(2,6), code, files, extractCode: extractCode||'', expiryDate: (expiryDate && !isNaN(Date.parse(expiryDate))) ? expiryDate : '', maxVisits: parseInt(maxVisits)||0, visitCount: 0, topHint: topHint||'', bottomHint: bottomHint||'', createdAt: new Date().toISOString(), createdBy: req.user.username, showNotification: showNotification !== false ? 'true' : 'false' };
    addShare(share);
    let link = getBaseUrl(req)+'/s/'+code;
...
app.get('/api/admin/shares', auth, (req, res) => {
  ...
  const items = all.slice(start, start+pageSize).map(s => ({
    ...s, link: getBaseUrl(req)+'/s/'+s.code,
    ...
    isDeleted: s.files && s.files.length > 0 && s.files.some(f => !fs.existsSync(f.path))
  }));
...
app.post('/api/admin/shares/cleanup', auth, (req, res) => {
  ...
  all.forEach(s => {
    ...
    const filesDeleted = s.files && s.files.length > 0 && s.files.some(f => !fs.existsSync(f.path));
    if (expired || filesDeleted) { deleteShare(s.id); removed++; }
  });
...
app.get('/api/admin/visits', auth, (req, res) => {
  ...
  var rows = dbAll('SELECT hour, COUNT(*) as cnt FROM visit_logs WHERE date=? GROUP BY hour ORDER BY hour', [date]);
  var hourly = [];
  for (var i = 0; i < 24; i++) hourly.push(0);
  rows.forEach(function(r) { hourly[r.hour] = r.cnt; });
  res.json({ date: date, hourly: hourly });
});

app.get('/api/admin/visits/calendar', auth, (req, res) => {
  var d = new Date();
  var year = parseInt(req.query.year) || d.getFullYear();
  var month = parseInt(req.query.month) || (d.getMonth() + 1);
  var monthStr = year + '-' + String(month).padStart(2,'0');
  var rows = dbAll("SELECT date, COUNT(*) as cnt FROM visit_logs WHERE date LIKE ? GROUP BY date ORDER BY date", [monthStr + '%']);
  var daily = {};
  rows.forEach(function(r) { daily[r.date] = r.cnt; });
  res.json({ year: year, month: month, daily: daily });
});
...
app.use('/api/admin/templates', (req, res, next) => {
  try { fs.mkdirSync(TEMPLATES_DIR, { recursive: true }); } catch(e) {}
  next();
});

// List all customizable templates
app.get('/api/admin/templates', auth, (req, res) => {
  const pages = listTemplateFiles();
  res.json({ success: true, pages });
});

// Get a template's content
app.get('/api/admin/templates/:name', auth, (req, res) => {
  const name = path.basename(req.params.name); //安全，防路径穿越
  const html = getTemplateContent(name);
  const overridden = fs.existsSync(path.join(TEMPLATES_DIR, name));
  res.json({ success: true, name, displayName: getTemplateDisplayName(name), content: html, overridden });
});

// Save template override
app.put('/api/admin/templates/:name', auth, (req, res) => {
  const name = path.basename(req.params.name);
  const { content } = req.body;
  if (typeof content !== 'string') {
    return res.status(400).json({ success: false, error: '缺少 content 字段' });
  }
  try {
    fs.mkdirSync(TEMPLATES_DIR, { recursive: true });
    fs.writeFileSync(path.join(TEMPLATES_DIR, name), content, 'utf8');
    logger.info('Template saved', { template: name, size: content.length });
    res.json({ success: true, message: getTemplateDisplayName(name) + ' 已保存，重启后生效' });
  } catch(e) {
...
app.post('/api/admin/templates/:name/reset', auth, (req, res) => {
  const name = path.basename(req.params.name);
  const overridePath = path.join(TEMPLATES_DIR, name);
  try {
    if (fs.existsSync(overridePath)) {
      fs.unlinkSync(overridePath);
    }
    logger.info('Template reset', { template: name });
    res.json({ success: true, message: getTemplateDisplayName(name) + ' 已恢复默认' });
  } catch(e) {
    res.status(500).json({ success: false, error: '重置失败: ' + e.message });
  }
});

// Restart the server
app.post('/api/admin/restart', auth, (req, res) => {
  res.json({ success: true, message: '正在重启应用…' });
  logger.info('Server restart requested by admin');
  // Graceful shutdown then restart via wrapper
  setTimeout(() => {
    process.exit(0);
  }, 500);
});
...
initDatabase().then(async () => {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    logger.info('FileShare started', { port: PORT, pid: process.pid });
    console.log(`┌─────────────────────────────────────────┐`);
    console.log(`│  FileShare System (SQLite)               │`);
    console.log(`│  Port:    ${PORT}                         │`);
    console.log(`│  PID:     ${process.pid}                  │`);
    console.log(`└─────────────────────────────────────────┘`);
    console.log('⚠️  请在管理后台立即修改默认密码 admin/admin123');
  });
}).catch(err => {
  logger.error('Failed to initialize', { message: err.message, stack: (err.stack || '').split('\n').slice(0,5).join('|') });
  process.exit(1);
});
